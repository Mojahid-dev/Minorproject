...
function ProfileForm({ user }: { user: ProfileUser }) {
  ...
  async function saveProfile() {
    ...
    setIsSaving(true);
    setStatus("");
    const { error } = await authClient.updateUser({ name: trimmedName, image: image || undefined });
    setIsSaving(false);
    setStatus(error ? error.message ?? "Could not save your profile." : "Profile saved successfully.");
  }
  ...
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 sm:p-6">
      <div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg bg-white text-black"><UserRound size={18} /></div><div><h2 className="font-semibold text-white">Profile</h2><p className="text-sm text-neutral-500">Update your public account details.</p></div></div>
      <div className="mt-5 border-t border-zinc-800 pt-5">
        <div className="flex flex-wrap items-center gap-4">
          <div className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-full bg-white text-xl font-bold text-black">
            {image ? <Image src={image} alt="Profile preview" width={80} height={80} unoptimized className="size-full object-cover" /> : initials(displayName)}
          </div>
          <div><label htmlFor="profile-photo" className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-zinc-700 px-3.5 py-2.5 text-sm font-semibold text-neutral-200 transition hover:bg-white/10 hover:text-white"><Camera size={16} /> Add profile photo</label><input id="profile-photo" type="file" accept="image/*" onChange={handleImageChange} className="sr-only" /><p className="mt-2 text-xs text-neutral-500">PNG, JPG, or WebP. Maximum 1 MB.</p></div>
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="block"><span className="text-sm font-medium text-neutral-200">Name</span><input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 text-sm text-white outline-none transition placeholder:text-neutral-600 focus:border-neutral-400 focus:ring-4 focus:ring-zinc-800" placeholder="Your name" /></label>
          <div><span className="text-sm font-medium text-neutral-200">Email</span><div className="mt-2 flex h-11 items-center rounded-xl border border-zinc-800 bg-zinc-950 px-3 text-sm text-neutral-500">{user.email}</div></div>
        </div>
        <div className="mt-5 flex flex-wrap items-center gap-3"><button type="button" onClick={saveProfile} disabled={isSaving} className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-black transition hover:bg-neutral-200 disabled:cursor-not-allowed disabled:opacity-60">{isSaving ? "Saving…" : "Save profile"}</button>{status && <p role="status" className="text-sm text-neutral-400">{status}</p>}</div>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-6 space-y-4">
          {isPending ? <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 text-sm text-neutral-500">Loading your profile…</section> : session?.user ? <ProfileForm user={session.user} /> : null}
